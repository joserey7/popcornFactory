...
class CatalogoActivity : AppCompatActivity() {
    ...
    var adapterS:SeriesAdapter ?= null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        movies_catalog.adapter = adapterP
        adapterS = SeriesAdapter(this, series)
        series_catalog.adapter = adapterS
    ...
    class SeriesAdapter: BaseAdapter {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            var series = series[position]
            var inflator = context!!.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
            var vista = inflator.inflate(R.layout.cell_movie, null)
            vista.image_movie_cell.setImageResource(series.image)
            vista.movie_title_cell.setText(series.titulo)

            vista.image_movie_cell.setOnClickListener{
                var intento = Intent(context, MovieDetail::class.java)
                intento.putExtra("titulo",series.titulo)
                intento.putExtra("sinopsis",series.sinopsis)
                intento.putExtra("header",series.header)
                intento.putExtra("image",series.image)
                context!!.startActivity(intento)
            }
            return vista
        }
